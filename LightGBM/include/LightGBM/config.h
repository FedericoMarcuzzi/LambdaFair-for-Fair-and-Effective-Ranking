...
struct Config {
  ...
  */
  ...
  // desc = set this to ``"ndcg"`` to enforce LambdaFair with NDCG+
  // desc = set this to ``"rnd"`` to enforce LambdaFair with rND+
  // desc = set this to ``"delta"`` to enforce LambdaFair with Delta-rND
  std::string lambda_fair = "plain";
...
inline std::string ParseMetricAlias(const std::string& type) {
  if (type == std::string("regression") || type == std::string("regression_l2") || type == std::string("l2") || type == std::string("mean_squared_error") || type == std::string("mse")) {
    ...
    return "ndcg";
  } else if (type == std::string("rnd")) {
    return "rnd";
  } else if (type == std::string("rnd1")) {
    return "rnd1";
